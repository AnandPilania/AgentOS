...
const on = (channel: string, cb: (...args: unknown[]) => void) => { const sub = (_: Electron.IpcRendererEvent, ...a: unknown[]) => cb(...a); ipcRenderer.on(channel, sub); return () => ipcRenderer.removeListener(channel, sub) }

contextBridge.exposeInMainWorld('agentos', {
    invoke, on, off: (channel: string, cb: unknown) => ipcRenderer.removeListener(channel, cb as never),
    ...
    auth: {
        login: (d: unknown) => invoke('auth:login', d),
        logout: () => invoke('auth:logout'),
        register: (d: unknown) => invoke('auth:register', d),
        me: (d: unknown) => invoke('auth:me', d),
        updatePrefs: (d: unknown) => invoke('auth:update-prefs', d),
    },
    settings: { get: () => invoke('settings:get'), set: (d: unknown) => invoke('settings:set', d), reset: () => invoke('settings:reset') },
    providers: { list: () => invoke('providers:list'), save: (d: unknown) => invoke('providers:save', d), test: (d: unknown) => invoke('providers:test', d) },
    audit: { list: (f: unknown) => invoke('audit:list', f), export: (f: unknown) => invoke('audit:export', f) },
    sessions: {
