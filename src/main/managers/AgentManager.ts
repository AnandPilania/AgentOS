...
export class AgentManager extends EventEmitter {
  ...
  // ─── Ollama ───────────────────────────────────────────
  private async runOllamaTurn(agent: Agent, history: AgentMessage[], _tools: AnthropicToolDef[], baseUrl: string, _wsPath?: string): Promise<void> {
    const messages = history
      .filter(m => m.role === 'user' || m.role === 'assistant')
      .map(m => ({ role: m.role, content: m.content }))

    // Add system prompt as first message if present
    if (agent.prompt && messages.length > 0) {
    ...
    try {
      ...
      throw new Error(`Cannot connect to Ollama at ${baseUrl}. Is Ollama running? (${err instanceof Error ? err.message : err})`)
    }
    ...
    while (true) {
      ...
      for (const line of decoder.decode(value, { stream:true }).split('\n')) {
        ...
        try {
          ...
          const t = j.message?.content ?? j.response ?? ''
          if (t) {
            text += t
            this.push('agent:message-chunk', { agentId:agent.id, chunk:t, msgId, done:false })
          }
    ...
    const finalText = text.trim() || '(empty response from Ollama)'
    const msg: AgentMessage = { id:uuid(), agentId:agent.id, role:'assistant', content:finalText, timestamp:new Date().toISOString() }
    this.saveMessage(msg)
    this.push('agent:message-chunk', { agentId:agent.id, message:msg, msgId, done:true })
  }
  ...
  searchMessages(query: string): Array<{agentId:string;message:AgentMessage}> {
    return this.db.all<Record<string,string>>(
      ...
      [`%${query}%`]
    ).map(r => ({ agentId:r.agent_id, message:{
      id:r.id, agentId:r.agent_id, role:r.role as AgentMessage['role'],
      content:r.content, timestamp:r.timestamp,
    }}))
  }
